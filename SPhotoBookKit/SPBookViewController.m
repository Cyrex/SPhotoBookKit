...
//
//  File name: SPBookViewController.m
...
typedef NS_ENUM(NSInteger, SPHalfBookCoverTag) {
    SPHalfBookCoverTagLeft  = 10,
    SPHalfBookCoverTagRight = 20
};


@interface SPBookViewController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate,UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) NSCache *reuseBookController;

@end

@implementation SPBookViewController
...
- (void)viewDidLoad {
    ...
    if (!self.startOpen) {
        ...
        UIView *closedPage = [self.bookCover viewWithTag:SPHalfBookCoverTagRight];
...
- (void)scrollBookViewControllerToIndex:(NSInteger)index {
    ...
    if (self.bookClosed) {
        ...
        NSInteger currentIndex = ((SPBookContentViewController *)(self.pageViewController.viewControllers.firstObject)).index;
        if (index > currentIndex) {
            [self.pageViewController setViewControllers:@[self.viewControllers[index],
                                                          self.viewControllers[index + 1]]
...
- (void)setUpBookCoverViewForFrontCover:(BOOL)front{
    ...
    if (front){
        halfBookCoverImageContainer = [self.bookCover viewWithTag:SPHalfBookCoverTagRight];
        [self.bookCover viewWithTag:SPHalfBookCoverTagLeft].hidden = YES;
        if (!halfBookCoverImageContainer){
            halfBookCoverImageContainer = [[UIView alloc] init];
            halfBookCoverImageContainer.tag = SPHalfBookCoverTagRight;
            ...
            if (nil == rightCoverImage) {
                rightCoverImage = [UIImage imageNamed:@"book_cover_right"];
            }
        ...
        [halfBookCoverImageContainer addConstraint:[NSLayoutConstraint constraintWithItem:coverImageView
                                                                                attribute:NSLayoutAttributeWidth
                                                                                relatedBy:NSLayoutRelationEqual
                                                                                   toItem:halfBookCoverImageContainer
                                                                                attribute:NSLayoutAttributeWidth
                                                                               multiplier:0.9
                                                                                 constant:0]];
        [halfBookCoverImageContainer addConstraint:[NSLayoutConstraint constraintWithItem:coverImageView
                                                                               ...
                                                                               multiplier:0.95
                                                                                 constant:0]];
        [halfBookCoverImageContainer addConstraint:[NSLayoutConstraint constraintWithItem:halfBookCoverImageContainer
        ...
        [halfBookCoverImageContainer addConstraint:[NSLayoutConstraint constraintWithItem:coverImageView
                                                                               ...
                                                                               multiplier:1
                                                                                 constant:0]];
        [self.bookCover viewWithTag:SPHalfBookCoverTagRight].hidden = NO;
    } else {
        [self.bookCover viewWithTag:SPHalfBookCoverTagRight].hidden = YES;
        halfBookCoverImageContainer = [self.bookCover viewWithTag:SPHalfBookCoverTagLeft];
        if (!halfBookCoverImageContainer){
            halfBookCoverImageContainer = [[UIView alloc] init];
            halfBookCoverImageContainer.tag = SPHalfBookCoverTagLeft;
            ...
            if (nil == leftCoverImage) {
                leftCoverImage = [UIImage imageNamed:@"book_cover_left"];
            }
        ...
        [self.bookCover viewWithTag:SPHalfBookCoverTagLeft].hidden = NO;
    }
}

- (void)setupCoverContentInView:(UIView *)halfBookCoverImageContainer{
    UIImageView *coverImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0,
                                                                                self.bookCover.frame.size.width / 2.0,
                                                                                self.bookCover.frame.size.height)];

    self.coverImageView = coverImageView;
    [self.dataSource bookViewController:self coverView:self.coverImageView];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController
    ...
    SPBookContentViewController *vc = (SPBookContentViewController *)viewController;
...
- (void)pageViewController:(UIPageViewController *)pageViewController
   ...
   previousViewControllers:(NSArray<UIViewController *> *)previousViewControllers
    ...
    self.animating = NO;
    if (completed){
        SPBookContentViewController *vc1 = [pageViewController.viewControllers firstObject];
        [UIView animateWithDuration:kBookAnimationTime/2.0 animations:^{
            if ([(SPBookContentViewController *)[previousViewControllers firstObject] index] < vc1.index){
                self.containerView.transform = CGAffineTransformIdentity;
...
- (UIPageViewController *)pageViewController {
    if (!_pageViewController) {
        ...
        [_pageViewController setViewControllers:@[self.viewControllers[0], self.viewControllers[1]]
                                      direction:UIPageViewControllerNavigationDirectionForward
                                       animated:NO
                                     completion:nil];
...
- (BOOL)isBookAtStart {
    SPBookContentViewController *vc1 = [self.pageViewController.viewControllers firstObject];
    return vc1.index == 0;
}

- (BOOL)isBookAtEnd {
    SPBookContentViewController *vc2 = [self.pageViewController.viewControllers lastObject];
...
- (void)openBook:(UIGestureRecognizer *)sender{
    ...
    [UIView animateWithDuration:kBookAnimationTime
                     animations:^{
                         if ([self.dataSource numberOfPhotoInBookViewController:self] != 0){
                             self.containerView.transform = CGAffineTransformIdentity;
                         }
                     } completion:nil];

    OLFlipStyle style = sender.view.tag == SPHalfBookCoverTagRight ? OLFlipStyleDefault : OLFlipStyleDirectionBackward;
...
- (void)closeBookBackForGesture:(UIPanGestureRecognizer *)sender{
    ...
    //Fade in shadow of the half-book.
    UIView *closedPage = [self.bookCover viewWithTag:SPHalfBookCoverTagLeft];
    CABasicAnimation *showAnim = [CABasicAnimation animationWithKeyPath:@"shadowOpacity"];
    showAnim.fromValue = [NSNumber numberWithFloat:0.0];
    showAnim.toValue = [NSNumber numberWithFloat:0.25];
    showAnim.duration = kBookAnimationTime/4.0;
...
- (void)onPanGestureRecognized:(UIPanGestureRecognizer *)recognizer{
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded){
        ...
        __weak SPBookViewController *welf = self;
        self.animating = YES;
        self.stranded = NO;
...
- (UIImageView *)bookImageView {
    if (!_bookImageView) {
        ...
        _bookImageView.image = [UIImage imageNamed:@"book_bottom_cover"];
    }
...
- (NSMutableArray *)viewControllers {
    if (!_viewControllers) {
        ...
        NSInteger count = [self.dataSource numberOfPhotoInBookViewController:self];
        for (int index = 0; index < count; index++) {
            [_viewControllers addObject:[self.dataSource bookViewController:self
                                              contentViewControllerForIndex:index]];
        }
    }
